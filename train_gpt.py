...
class Hyperparameters:
    ...
    skip_quant_roundtrip = bool(int(os.environ.get("SKIP_QUANT_ROUNDTRIP", "0")))
    sdp_backend = os.environ.get("SDP_BACKEND", "auto").strip().lower()
    enable_torch_compile = (
        os.environ.get("ENABLE_TORCH_COMPILE", "auto").strip().lower()
    )
    save_raw_checkpoint = bool(int(os.environ.get("SAVE_RAW_CHECKPOINT", "0")))
    raw_checkpoint_dir = os.environ.get("RAW_CHECKPOINT_DIR", "lab/checkpoints")
...
def main() -> None:
    ...
    if enable_torch_compile == "auto":
        use_torch_compile = sys.platform != "win32"
    elif enable_torch_compile in {"1", "true", "yes", "on"}:
        use_torch_compile = True
    elif enable_torch_compile in {"0", "false", "no", "off"}:
        use_torch_compile = False
    else:
        raise ValueError(
            f"Unsupported ENABLE_TORCH_COMPILE={args.enable_torch_compile!r}; expected auto, 0, or 1"
        )
    ...
    if args.grad_accum_steps > 0:
        ...
        if default_grad_accum_steps <= 0:
            raise ValueError(
                f"WORLD_SIZE={world_size} must divide 8 unless GRAD_ACCUM_STEPS is set explicitly"
            )
    ...
    if sdp_backend == "auto":
        sdp_backend = "flash" if sys.platform != "win32" else "math"
    if sdp_backend == "flash":
        sdp_flags = dict(cudnn=False, flash=True, mem_efficient=False, math=False)
    elif sdp_backend == "math":
        sdp_flags = dict(cudnn=False, flash=False, mem_efficient=False, math=True)
    elif sdp_backend == "flash_math":
        sdp_flags = dict(cudnn=False, flash=True, mem_efficient=False, math=True)
    else:
        raise ValueError(
            f"Unsupported SDP_BACKEND={args.sdp_backend!r}; expected auto, flash, math, or flash_math"
        )

    enable_cudnn_sdp(sdp_flags["cudnn"])
    enable_flash_sdp(sdp_flags["flash"])
    enable_mem_efficient_sdp(sdp_flags["mem_efficient"])
    enable_math_sdp(sdp_flags["math"])
    ...
    log0(
        f"sdp_backends:cudnn={sdp_flags['cudnn']} flash={sdp_flags['flash']} "
        f"mem_efficient={sdp_flags['mem_efficient']} math={sdp_flags['math']}"
    )
    log0(
    ...
    log0(f"torch_compile:{use_torch_compile}")
    ...
    if last_val_loss is None or last_val_bpb is None:
        raise RuntimeError("Expected a final validation pass before serialization")

    log0(
        f"final_prequant_exact val_loss:{last_val_loss:.8f} val_bpb:{last_val_bpb:.8f}"
    )
    ...
    if master_process and args.save_raw_checkpoint:
        raw_checkpoint_dir = Path(args.raw_checkpoint_dir)
        raw_checkpoint_dir.mkdir(parents=True, exist_ok=True)
        raw_checkpoint_path = raw_checkpoint_dir / f"{args.run_id}.pt"
        torch.save(
            {
                "run_id": args.run_id,
        ...
        model_bytes = raw_checkpoint_path.stat().st_size
        log0(
            f"Saved raw checkpoint: {raw_checkpoint_path.as_posix()} ({model_bytes} bytes)"
        )
    if args.skip_quant_roundtrip:
        log0(
            "Skipping final int8+zlib export and roundtrip eval because SKIP_QUANT_ROUNDTRIP=1"
        )
    else:
        if master_process:
        ...
        if master_process:
            ...
            log0(
                f"Total submission size int8+zlib: {quant_file_bytes + code_bytes} bytes"
            )
    ...
    total_seconds = time.perf_counter() - run_wallclock_start
    total_submission_bytes_int8 = (
        quant_file_bytes + code_bytes if quant_file_bytes >= 0 else -1
    )
    log0(
        "lab_summary "
        f"steps:{step} train_batch_tokens:{args.train_batch_tokens} train_seq_len:{args.train_seq_len} "
        f"grad_accum_steps:{grad_accum_steps} training_seconds:{training_time_ms / 1000.0:.3f} "
        f"total_seconds:{total_seconds:.3f} peak_alloc_mib:{peak_alloc_mib} peak_reserved_mib:{peak_reserved_mib} "
        f"prequant_val_loss:{last_val_loss:.8f} prequant_val_bpb:{last_val_bpb:.8f} "
        f"quantized_val_loss:{q_val_loss:.8f} quantized_val_bpb:{q_val_bpb:.8f} "
        f"code_bytes:{code_bytes} model_bytes_raw:{model_bytes} model_bytes_int8_zlib:{quant_file_bytes} "
        f"total_submission_bytes_int8_zlib:{total_submission_bytes_int8}"
    )
